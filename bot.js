...
client.on('ready', () => { 
  ...
  .addField(`Room`,`<#${message.channel.id}>`,true)
  .addField(`Old`,`**${message.cleanContent}**`,true)
  .addField(`New`,`**${newMessage.cleanContent}**`)
  .setTimestamp()
     channel.send(embed);
  ...
  client.on("guildMemberAdd", member => {
  const shit = member.guild.channels.find("name","welcome");       
  var Canvas = require('canvas')
  var jimp = require('jimp')
  const mi = ['./wlc/guildAdd.png'];
  let Image = Canvas.Image,
  canvas = new Canvas(430, 120),
  ...
  ctx.drawImage(ground, 0, 0, 430, 120);
  })
  ...
  ctx.fillText(`${member.user.username}`, 230, 70);
  let Avatar = Canvas.Image;
  let ava = new Avatar;
  ava.src = buf;
  ctx.beginPath();
  ctx.drawImage(ava, 14, 11, 100, 100)
  
  shit.sendFile(canvas.toBuffer())
  });
  });
  });
  ...
  const shit = member.guild.channels.find("name","out");   
  var Canvas = require('canvas')
  var jimp = require('jimp')
  const mi = ['./wlc/guildRemove.png'];
  let Image = Canvas.Image,
  canvas = new Canvas(430, 120),
  ctx = canvas.getContext('2d');
  ctx.patternQuality = 'bilinear';
